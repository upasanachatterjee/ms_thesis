...
We include the current state-of-the-art system for article-level political bias classification, the POLITICS model developed by \citet{liu2022politics}. This system represents a RoBERTa-based encoder enhanced through continued pre-training using a contrastive triplet-loss framework, where $\langle$anchor, positive, negative$\rangle$ triplets generated with the following example sampling procedure:
...
where $f(\cdot)$ denotes the model's [CLS] token representation, and $\delta$ is a margin hyperparameter. The POLITICS model incorporates both story-level objectives (positive samples discuss the same event) and ideology-level objectives (positive samples share political orientation, as in the example above) during continued pre-training.
