...
\paragraph{Why F1 cannot detect this on its own.} It is worth being explicit about why standard accuracy metrics are blind to the question we are about to ask. F1, accuracy, and per-class precision/recall all measure the input--output mapping of a classifier: given an article, does the predicted label match the gold label? They evaluate what the model produces, not which features the model uses to produce it. Table~\ref{fig:khalil} makes this concrete: AP and Fox News cover the Khalil arrest with identical topic tags but sharply divergent framing, and two classifiers could agree on the ideology labels for both articles while relying on completely different internal pathways. One classifier might route its predictions through a human-aligned chain of framing, topical emphasis, and stance, recognizing the same linguistic cues a human annotator would weigh. The other might route its predictions through a spurious sentiment-only shortcut that happens to correlate with ideology in the training distribution, latching on to affective polarity without engaging the framing structure at all. In-distribution F1 cannot tell these apart, because both pathways produce the same labels on the same articles. A model can therefore achieve a high F1 by pushing predictions through a spurious sentiment pathway whose causal structure diverges sharply from the human pathway, and the classification scoreboard will register success either way. Causal probing fills exactly this gap: rather than asking whether the labels match, it asks whether the causal pathway from sentiment and topic to ideology matches the human pathway, and surfaces shortcut learning that F1 by construction cannot.

The rest of this chapter presents a causal inference framework applied to the AllSidesV2 Media test set. We formalize the causal structure using a directed graph, describe our Double Machine Learning (DML) methodology, and report results at two levels of analysis across five annotation paradigms: human labels, a fine-tuned RoBERTa classifier, zero-shot GPT-4o-mini, fine-tuned GPT-4o-mini, and zero-shot Llama-3.3-70B. We present community-level results first, as these yield the strongest and most interpretable effects. At the community level, only the fine-tuned GPT-4o-mini produces significant effects (for Social Issues, Geopolitics, and Trump Presidency communities) while human, Fine-tuned RoBERTa, and Llama-3.3-70B annotations remain null. Llama-3.3-70B exhibits a distinct failure mode: rather than producing sign reversals like GPT models, it systematically attenuates all effect sizes toward zero. We then present topic-level results for completeness: no robust causal effects emerge at this granularity, significance is paradigm-dependent, and estimated causal directions can reverse depending on the annotation model used. We interpret the fine-tuned GPT divergence as evidence of shortcut learning: fine-tuning introduces spurious sentiment-ideology coupling that is invisible to standard F1-based evaluation but surfaces under causal probing.
