...
\begin{table}[h]
    ...
    \begin{minipage}{0.45\textwidth}
        ...
        \end{tabular}
    \end{minipage}
    \caption{Top five communities and topic tags in the AllsidesV2 media split test set. Total article count is 11,516. The full community structure with all 15 communities and their constituent topics is provided in Appendix~\ref{appendix:community-structure}.}
...
For community-level mediation, we restrict the analysis to articles that contain topics from \emph{both} the treatment community and the mediator community. Returning to Figure~\ref{fig:articles}, this means selecting only articles whose topic tags span at least two community colors, those where the treatment community's sentiment and the mediator community's sentiment are both observable. This subsetting can substantially reduce the sample size: of the \num{9830} articles in the AllSidesV2 Media test set with sentiment annotations, only 139 contain topics from both Community~9 (Geopolitics and Foreign Affairs) and Community~11 (Trump Presidency). However, this focused subpopulation reveals much larger per-unit effects than the global ATE, because the global estimate averages a concentrated subgroup effect across the majority of articles where the treatment community's topics are absent entirely (see Section~\ref{sec:community-mediation} for detailed results).
