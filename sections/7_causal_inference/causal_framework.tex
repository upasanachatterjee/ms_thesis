...
We use the AllSidesV2 dataset described in Section~\ref{sec:dataset}. All causal analyses are performed on the test set of the AllSidesV2 Media split (\num{11516} articles) to ensure fair comparison across annotation sources and to avoid any overlap with transformer fine-tuning data. To extract sentiment variables ($F$), we apply a two-step process to the full article text ($X$):
...
\begin{figure}
  ...
  \caption{Articles with predicted ideologies, topics, sentiments, and topic clustering.}
  \label{fig:articles}
\end{figure}
...
We focus on the five most frequent topic tags (Table~\ref{tab:top-tags}), which provide sufficient statistical power while spanning three distinct communities: political figures and electoral processes (Community 11: \textit{donald\_trump}, \textit{elections}), institutional politics (Community 2: \textit{politics}, \textit{white\_house}), and international affairs (Community 4: \textit{world}).
...
Table~\ref{tab:llm-causal-performance} summarizes the classification performance of each LLM paradigm on the AllSidesV2 test set.
...
\begin{table}[h]
    ...
    \bottomrule
    \end{tabular}
    \caption{LLM classification performance on AllSidesV2 test set. Fine-tuned RoBERTa F1 is reported in Section~\ref{sec:continued-pretraining}.}
