...
All experiments are conducted across five annotation paradigms:
...
\begin{enumerate}
    ...
    \item \textbf{GPT-4o-mini (baseline):} Zero-shot predictions from GPT-4o-mini without task-specific fine-tuning.

    \item \textbf{Fine-tuned GPT-4o-mini:} Predictions from the fine-tuned GPT model from Section~\ref{sec:llm-finetuning}.

    \item \textbf{Llama-3.3-70B:} Zero-shot predictions from Meta's Llama-3.3-70B-versatile model without task-specific fine-tuning. This model is included to test whether causal patterns generalize across LLM families or are specific to the GPT architecture.
\end{enumerate}
...
For community-level mediation, we restrict the analysis to articles that contain topics from \emph{both} the treatment community and the mediator community. Returning to Figure~\ref{fig:articles}, this means selecting only articles whose topic tags span at least two community colors, those where the treatment community's sentiment and the mediator community's sentiment are both observable. This subsetting can substantially reduce the sample size: of the \num{11516} articles in the full corpus, only 139 contain topics from both Community~9 (Geopolitics and Foreign Affairs) and Community~11 (Trump Presidency). However, this focused subpopulation reveals much larger per-unit effects than the global ATE, because the global estimate averages a concentrated subgroup effect across the majority of articles where the treatment community's topics are absent entirely (see Section~\ref{sec:community-mediation} for detailed results).
